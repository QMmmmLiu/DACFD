...
Imagine a scenario where we wish to test the difference in cognitive ability in people of different age group. In this toy example, we have 1000 participants for three age groups, the younger-aged group has true mean and variance of respectively 30 and 25, the middle-aged group 20 and 25 and the older 10 and 100. The higher the score, the higher the cognitive ability. We can check the mean and variance of the true mean and variance on the data composed of true scores, `ca.true`. 
...
Now consider the fact that a substantial proportion of the younger-aged group may score maximum at the cognitive ability test and a substantial proportion of the older-aged group may score minimum. Let both the ceiling and the floor proportion be 15%, we have the dataset `ca.cf`.
...
Now say we wish to conduct an ANOVA in the data with floor and ceiling effects. We can use the function `lw.f.star`. We can also conduct t-test, say between the older-aged and the younger-aged group by using the function `lw.t.test`. However, we need to choose between the two proposed methods: method `a` make more accurate effect size estimate, `b` is more conservative in terms of hypothesis testing. 
