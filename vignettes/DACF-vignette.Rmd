...
Now we wish to conduct an ANOVA in the data with floor and ceiling effects. We can use the function `lw.f.star`. We can also conduct a t-test between the older-aged and the younger-aged group by using the function `lw.t.test`. However, we need to choose between the two proposed methods: method `a` yields more accurate effect size estimate but can be slightly too liberal in hypothesis testing, `b` has well-controlled type I error rates but can be slightly conservative. 
